...
void RellenarTableroRecibido(int *_posicion)
{
  ...
  _tablero[fil][col] = 3; //Imprimira un 0
}

int Ganador(int pos_uno, int pos_dos, int pos_tres)
{
    if(pos_uno == _numJugador && pos_dos == _numJugador && pos_tres == _numJugador)
    {
        system("clear");
        printf("¡¡¡HAS GANADO!!! \n");
...
int main(int argc, char const *argv[]) {
  ...
  const char * const socket_nombre = argv[1];
  ...
  pthread_create(&idHilo, NULL, ConectarAlOtro, NULL);
